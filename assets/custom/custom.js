...
class Theme{
    getMode(){
        return $('body').data('bs-theme') ?? 'light'
    }
};
...
const badge = (html = '', my_class = 'info') => {
    return `<label class="badge bg-${my_class}">${html}</label>`;
}
const currentYear = () => {
...
const avatar = (img,class_name = 'sm') => {
    return `
    <a data-aryalightbox="gallery" href="${base_url}upload/${img}">
    <span class="avatar avatar-${class_name}" style="background-image: url(${base_url}upload/${img})"></span></a>`;
}
...
const mydrawer = (title) => {
    ...
    drawerEl.addEventListener("hidden.bs.offcanvas", function () {
        ...
        main.find('.offcanvas-body').html('');
        main.find('.offcanvas-footer').remove();
        ...
        function callAjax(settings) {
            ...
            AryaAjaxXhr = $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    SwalHideLoading();
                    if (status) {
                        if (typeof KTDrawer == 'function' || true) {
                            var div = mydrawer(error).find('.body').html(xhr.responseText);
                            div.find('div > h4').addClass('text-danger');
                            div.find('#container > h1').addClass('text-danger');
                        }
                        else {
                            SwalWarning(error, xhr.responseText);
                        }
...
$(document).on('click', '.advanced-set-page', function () {
        ...
        body.append(`<div class="py-1">
                        <div class="rounded border border-primary p-2">
                            ...
                            <div class="row">
                                <!--begin::Label-->
                                <div class="col">
                                    <label class="fs-3 fw-semibold form-label m-0" for="my_page_${page.id}">${page.title}</label>
                                </div>
                                <!--end::Label-->
                                <!--begin::Switch-->
                                <label class="col-auto m-0 form-check form-switch form-check-custom form-check-solid">
    ...
    body.find('[data-kt-search-element="form"] input').on('input', function () {
        ...
        if (searchText.trim() == '')
            $('[data-kt-search-element="clear"]').addClass('fade');

        else
            $('[data-kt-search-element="clear"]').removeClass('fade');
...
$(document).on('click','[data-aryalightbox="gallery"]',function(e){
    e.preventDefault();
