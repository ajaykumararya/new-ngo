...
class Ki_theme
{
    ...
    function back_button($url, $text = 'Back',$icon = 'arrow-left',$class = 'warning')
    {
        return $this->with_icon($icon)->with_pulse($class)->outline_dashed_style($class)->set_class('text-')->add_action($text, $url);
    }
    ...
    function generateMenu($menuItems, $type = 'menu', $menuType = '')
    {
        ...
        foreach ($menuItems as $menuItem) {
            ...
            if ($type == 'menu' && isset($menuItem['icon'])) {
                ...
                $icon = '<span class="nav-link-icon d-md-none d-lg-inline-block"><i class="fa fa-' . $icon . '"></i></span>';
            }
            $id = '';
            $text = '<span class="nav-link-title">' . $menuItem['label'] . '</span>';
            if ($type == 'submenu') {
                $html .= '<a class="dropdown-item '.($activeMenu ? 'active' : '').'" href="' . (isset($menuItem['submenu']) ? '#' : $this->gen_link(@$menuItem['url'])) . '">' . $text . '</a>';
            } else {
                if (isset($menuItem['submenu'])) {
                    $id = 'sidebar' . mt_rand(0, 999);


                    $html .= '<li class="'.($menuItemActive ? 'active' : '').' nav-item dropdown"><a class="nav-link dropdown-toggle" href="#navbar-base" data-bs-toggle="dropdown" data-bs-auto-close="outside" role="button" aria-expanded="false">
                            ' . $icon . '' . $text . '<span class="menu-arrow"></span>';
                } else
                    $html .= '<li class="'.($activeMenu ? 'active' : '').' nav-item"><a class="nav-link" href="' . (isset($menuItem['submenu']) ? '#' : $this->gen_link(@$menuItem['url'])) . '">
                                    ' . $icon . '' . $text;
            }
            $html .= '</a>';
            if (isset($menuItem['submenu'])) {
